...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName("steamid")
        .setDescription("Obtiene información detallada de un SteamID")
    ...
    async execute(interaction) {
        ...
        try {
            ...
            dbConfig = await ServerConfig.findOne({
            ...
            return await interaction.editReply(
                "❌ Error al consultar la configuración del servidor."
            );
        }
        ...
        if (!dbConfig || !dbConfig.steamIdApiKey) {

            return await interaction.editReply(
                "❌ Este servidor no tiene configurada la API Key de SteamID.uk."
            );

        }

        // ==========================================
        // STEAMID PROPIETARIO DE LA API
        // ==========================================

        const myId =
            dbConfig.steamIdMyId ||
            process.env.STEAMID_MYID;

        if (!myId) {

            return await interaction.editReply(
                "❌ Este servidor no tiene configurado el SteamID propietario de SteamID.uk."
            );
